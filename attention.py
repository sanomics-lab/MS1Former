...
class MultiHeadAttention(nn.Module):
    ...
    def forward(self, inputs):
        ...
        """
        q, k, v = map(
            lambda x: rearrange(x, "b l (h c) -> b h l c", h=self._num_heads), (q, k, v)
        )
        ...
        rel_logits = einsum(
            "b h i c, h j c -> b h i j", q + self._rel_pos_bias, rel_k
        )  # [B, H, L, 2L-1]
        rel_logits = relative_shift(rel_logits)  # [B, H, L, L]

        content_logits = einsum(
            "b h i c, b h j c -> b h i j", q + self._rel_content_bias, k
        )  # [B, H, L, L]
...
def positional_features_exponential(
    positions, feature_size, seq_length, min_half_life=3.0
):
    """
...
def positional_features_central_mask(positions, feature_size, seq_length):
    ...
    center_widths = (
        2 ** torch.arange(1, feature_size + 1, device=positions.device).float()
    )
    center_widths = center_widths - 1
    output = (center_widths[None, ...] > positions.abs()[..., None]).float()
    assert output.shape[:-1] == positions.shape and output.shape[-1] == feature_size
    return output
...
def positional_features_gamma(
    positions, feature_size, seq_length, stddev=None, start_mean=None, eps=1e-8
):
    if stddev is None:
    ...
    mean = torch.linspace(start_mean, seq_length, feature_size, device=positions.device)
    mean = mean[None, ...]
...
def positional_features_all(positions, feature_size, seq_length, symmetric=False):
    ...
    assert feature_size % num_components == 0, (
    ...
    embeddings = [
        f(torch.abs(positions), num_basis_per_class, seq_length)
    ...
    if not symmetric:
        embeddings = torch.cat(
            embeddings, torch.sign(positions)[..., None] * embeddings, dim=-1
        )
    return embeddings
