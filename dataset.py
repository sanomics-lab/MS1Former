    ...
    for idx,spectrum in enumerate(rawdata_reader):
        if spectrum[mslevel_string] ==1:
            ...
            if spectrum["index"] > index_win[0] and spectrum["index"] < index_win[1]:
                ms2.rt_list.append(60*spectrum["scanList"]["scan"][0]["scan start time"])
                ms2.indexs.append(spectrum["index"])
                precursor_mz = spectrum["precursorList"]["precursor"][0]["selectedIonList"]["selectedIon"][0]['selected ion m/z']
                precursor_intensity = spectrum["precursorList"]["precursor"][0]["selectedIonList"]["selectedIon"][0]['peak intensity']
                precursor_leftBound = spectrum["precursorList"]["precursor"][0]["isolationWindow"]["isolation window lower offset"]
                precursor_rightBound = spectrum["precursorList"]["precursor"][0]["isolationWindow"]["isolation window upper offset"]
...
def process_spectrum(spectrum_mz_ls,spectrum_intensity_ls,config):
    ...
    charge = 1.0
    spectrum_mz_cutted_ls = [mz for mz in spectrum_mz_ls if mz >= config.mz_min and mz <= config.mz_max]
    spectrum_mz = np.array(spectrum_mz_cutted_ls,dtype=np.float32)
    neutral_mass = spectrum_mz - charge*config.mass_H
    neutral_mass_location = np.rint((neutral_mass-config.mz_min) * config.spectrum_resolution).astype(np.int32)
    # normalize intensity
    ...
    for index in range(neutral_mass_location.size):
        ...
        if spectrum_holder[neutral_mass_location[index]] ==0:
            ...
            spectrum_holder[neutral_mass_location[index]] = max(spectrum_holder[neutral_mass_location[index]],norm_intensity[index])
...
def main_function(config):
    ...
    for file in tqdm(os.listdir(config.file_dir)):
        if file.endswith(".mzML"):
            if os.path.exists(os.path.join(config.save_dir,file.split(".")[0]+".pt")):
